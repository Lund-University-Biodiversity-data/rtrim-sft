  ...
  if(nsyst>1){
    ...
    for (c in 1:length(combinations)) {            # length can only be 1 currently
      ...
      write_xlsx(list(TRIMCOMBO=combtab[order(combtab$ART),]),
                  path = paste0(path_project_extract,'Trimcombined_', paste(colnames[cix], collapse = '_'), '-', min(combtab$Yr), '_', max(combtab$Yr), '_Figurritning_', gsub('[ :]', '_', round(Sys.time(), 0)), '.xlsx'),
                  format_headers = TRUE)
